...
def merge_followup_state_into_tool_state(
    ...
    if not merged_context.get("last_tool_name") and followup_context.get("last_tool_name") is not None:
        merged_context["last_tool_name"] = deepcopy(followup_context["last_tool_name"])
    if not merged_context.get("last_tool_output") and followup_context.get("last_tool_output") is not None:
        merged_context["last_tool_output"] = deepcopy(followup_context["last_tool_output"])
    if not merged_context.get("error_message") and followup_context.get("error_message") is not None:
        merged_context["error_message"] = deepcopy(followup_context["error_message"])
...
def cleanup_world_model_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    while cursor < len(messages):
        ...
        if message.get("role") == "action" and not is_tool_action_content(message.get("content")):
            ...
            if next_message and next_message.get("role") == "state":
                cursor += 2
            else:
                cursor += 1
            continue
