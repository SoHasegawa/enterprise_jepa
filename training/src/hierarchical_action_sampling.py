...
src/beam_plan.py with a hierarchical-latent-action Cross-Entropy Method (CEM):
the LLM opens the search space ONCE per planning cycle with a small set of
diverse anchor actions; the world model then samples thousands of CONTINUOUS
latent-action trajectories around those anchors, batch-rolls them out in
latent space (no LLM, no text decode), and iteratively refines a per-family
Gaussian proposal distribution toward the highest-scoring region. Only the
converged best trajectory is decoded back to an executable action.

Two kinds of randomness are deliberately kept separate (do not conflate them):
  * SigReg (elsewhere in finetuning_jepa.py) pulls the STATE embedding z toward
    an isotropic Gaussian to prevent representation collapse.
  * This module's CEM samples ACTION trajectories from a Gaussian PROPOSAL
    distribution that is refit every iteration toward the elites -- a search
    procedure, not a regularizer.

Action "family" (categorical layer): every action here arrives as an MCP-style
tool call, but the underlying backend varies by benchmark. Plain MCP tools
(EnterpriseOps-Gym, ...) use family = tool name -- interpolating between two
`create_event` calls' argument latents is meaningful (parameter variation);
interpolating between `create_event` and `get_calendar_list` is not. SQL and
shell backends (CRMArenaPro's `execute_crm_sql`, TerminalBench's
`execute_bash`/`exec`) wrap free-form query/command TEXT inside one dominant
tool name, so tool-name-as-family would collapse every distinct SQL statement
or shell command into a single bucket -- exactly the degenerate case the
family layer exists to prevent. `_family_key` detects these backends (SQL
keyword in `arguments.query`/`sql`/`statement`, or a shell-hinting tool name
with `arguments.command`/`cmd`/`script`) and uses the operation VERB (SQL
keyword; base shell command, unwrapping `bash -lc '...'` and taking only the
first pipeline segment) as the family instead.

Explicit bindings (ids, paths, emails, literals) are NEVER sampled from the
continuous latent. Two decode strategies are available (`HierarchicalCEMConfig
.decode_strategy`):
  * "nearest_anchor" (default): the *choice* of which LLM-proposed anchor to
    execute is guided by the latent search; the anchor's own bindings are used
    verbatim, never interpolated. Always valid (it's a real anchor), but can
    only ever recover an action the LLM already proposed.
  * "learned_decoder": decodes the CEM-sampled/interpolated latent directly via
    TextLeWorldModel.decode_action_latent (trained with --action-decoder-loss-
    coeff; see finetuning_jepa.py). This is what actually realizes "explore an
    interpolated point, not just re-rank LLM anchors" -- but a decoder trained
    on real anchors is not guaranteed to produce a valid action for an
    arbitrary latent, so every decode is passed through `validate_decoded_action`
    and falls back to nearest_anchor on failure.

Fast-LeWorldModel's action-prefix parallel predictor is intentionally NOT used
here -- rollout is the existing recursive one-step-at-a-time
`predict_latent` chain (batched over all N candidates), matching the
autoregressive CEM in the original LeWorldModel paper.
"""
...
# SQL/shell backends (CRMArenaPro, TerminalBench) wrap free-form query/command text inside a
# single dominant tool ("execute_crm_sql", "execute_bash"/"exec"), so tool-name alone collapses
# every distinct SQL statement or shell command into ONE family -- exactly the degenerate case
# the family layer exists to avoid. Detect these backends and use the operation VERB (SQL
# keyword; base shell command) as the family instead, falling back to tool name for genuine
# MCP tool calls (where the name already IS the operation).
...
# Python-execution, browser-action, and SWE-code-edit backends (ADP: codeactinstruct,
# go-browse-wa/nnetnav/mind2web/synatra, swe-smith/openhands/swe-play/...). No live
# execution harness targets these yet in this repo (beam_plan/hier_latent_cem currently only
# run against EnterpriseOps-Gym), but the family layer is generalized ahead of that so it's
# ready when one exists -- verified against real ADP action payloads (see exploration
# findings): every group already arrives as the same {"tool_calls":[...]} shape, so no new
# JSON-parsing is needed, only a name/argument -> family mapping.
...
# Playwright/browser-agent verb vocabulary (go-browse-wa, nnetnav, mind2web, synatra): the
# tool NAME already IS the operation here (like plain MCP), so no argument-digging is
# needed -- just recognizing these names as belonging to the "browser" backend rather than
# an arbitrary MCP tool, for display/grouping consistency with the other backends.
_BROWSER_VERBS = {
...
def _shell_command_verb(command: str) -> str:
    ...
    shell string: strip the outer wrapper, take only the FIRST pipeline segment (later
    segments are follow-up commands, not this action's primary effect), then the first
    token that isn't an env-var assignment or a no-op prefix (sudo/env/nohup/time)."""
    text = command.strip()
...
def _python_verb(code: str) -> str:
    ...
    cheap analogue of the shell-verb heuristic for arbitrary Python/pandas/sql-hybrid
    snippets (import/def/class/for/df/pd/plt/...). Not a parser; good enough to separate
    "this snippet mostly imports" from "this snippet mostly plots" as distinct families."""
...
def _family_key(action: dict[str, Any]) -> str:
    ...
    if command is not None and any(hint in name_lower for hint in _SHELL_NAME_HINTS):
        # A generic "command" argument key is not unambiguous on its own (an ordinary MCP
        # tool could plausibly have one) -- require the tool-name hint too.
    ...
    if edit_command is not None and any(hint in name_lower for hint in _CODE_EDIT_NAME_HINTS):
        ...
        # expose a clean sub-operation enum (view/str_replace/create/insert/undo_edit) --
        # no text parsing needed, just read it.
    ...
    if name_lower in _BROWSER_VERBS:
        # The tool name already IS the operation (like plain MCP) -- just tag the backend
        # for grouping/display consistency with sql:/shell:/python:/code_edit:.
        return "browser:" + name_lower

    # Agent-env-text verbs (alfworld: go/take/put/open/...) don't map cleanly onto
    # MCP/SQL/shell/Python/browser -- the tool name already IS the operation there too, so
    # the plain tool-name fallback below is correct and sufficient, same as real MCP tools.
    names = sorted(
...
def build_anchor_prompt(
    ...
    from src.beam_plan import BeamPlanConfig, build_step_candidates_prompt
...
def parse_anchor_actions(text: str, config: HierarchicalCEMConfig) -> list[dict[str, Any]]:
    from src.beam_plan import BeamPlanConfig, parse_action_candidates
...
def _rollout_and_score(
    ...
    for t in range(horizon):
        z_action = u[:, t, :]
        # State-input canonical heads (--canonical-event-head-inputs state) read the
        # predictor's hidden state; None under other modes and ignored there.
        ...
        # One host transfer per field instead of one per probability scalar -- at N=256 CEM
        # samples the per-scalar version is thousands of device syncs per horizon step.
...
def _decode_predicted_state(field_probs: dict[str, dict[str, float]]) -> dict[str, Any]:
    """Same convention as finetuning.score_action_plans_canonical_event: argmax per
    single-label field, >=0.5 threshold for the multi-label field -- so beam_plan and this
    module's imagined-plan entries render identically in the orchestrator's injected message."""
...
def _format_step_reason(contributions: dict[str, float], top_fields: int = 3) -> str:
    ...
    canonical_event_scoring.explain_trajectory but scoped to a single horizon step."""
...
def validate_decoded_action(text: str) -> dict[str, Any] | None:
    ...
    doesn't have -- see the design memo's L_valid) -- only rejects obviously-broken decodes
    (empty, unparseable JSON with no recognizable tool call, or truncated garbage) before an
    action is ever handed to the caller for execution. Returns the normalized action dict, or
    None if the decode should be discarded (caller falls back to nearest_anchor)."""
...
def _decode_learned_action(
    ...
    fails validate_decoded_action. `tool_label` -- the family/tool this latent was sampled FROM,
    already resolved to the training-time tool_vocab id by the caller -- is handed to the
    decoder's tool-conditioning slot when the checkpoint has one (None/unmapped families decode
    with the "unknown tool" row, same as an untrained decoder would)."""
    if not getattr(net, "action_decoder", False):
        return None
    try:
        texts = net.decode_action_latent(
            u_step.unsqueeze(0), tokenizer, max_new_tokens=config.decode_max_new_tokens, tool_label=tool_label,
        )
    except Exception:  # noqa: BLE001 -- a broken decode must never crash planning
        return None
...
def hierarchical_cem_plan(
    ...
    tool_vocab: dict[str, int] | None = None,
) -> dict[str, Any]:
    ...
    `tool_vocab` -- the {tool_name: idx} mapping the checkpoint's action_decoder tool-
    conditioning embeddings were trained against (finetuning_jepa.build_tool_vocabulary,
    persisted as tool_vocab.json) -- is only consulted when decode_strategy=="learned_decoder";
    a family string with no entry (e.g. a generalized "sql:select"/"shell:ls" bucket that never
    appeared as a literal MCP tool name at training time) falls back to the decoder's "unknown
    tool" row, same as passing no vocab at all."""
    ...
    for _ in range(max(1, config.num_iters)):
        ...
        trajectories = _rollout_and_score(net, z_current_batch, z_context_batch, u, config, vocab, goal_batch)
    ...
    for t in range(config.horizon):
        ...
        if config.decode_strategy == "learned_decoder":
            tool_label = tool_vocab.get(best_family_ids[t]) if tool_vocab else None
            decoded = _decode_learned_action(net, tokenizer, best_u[t], config, tool_label=tool_label)
