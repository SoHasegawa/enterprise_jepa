...
def _resolve_purple_agent_version() -> str:
    ...
    if version is None:
        raise RuntimeError("crmarenapro purple agent version is not configured")
    return version


def default_executor_name() -> str:
    return (
        os.getenv("CRMARENAPRO_EXECUTOR")
...
def _resolve_executor_version(executor_name: str) -> str | None:
    return load_component_version(
        Path(__file__).resolve().parent.parent / "purple-executors" / executor_name,
        env_name="BENCHMARK_EXECUTOR_VERSION",
    )


def build_agent_card(host: str, port: int, card_url: str | None, executor_name: str) -> AgentCard:
    skills = [
        AgentSkill(
            id=f"crmarenapro_{executor_name}",
            name=f"crmarenapro_{executor_name}",
            description=f"CRMArenaPro purple wrapper for executor '{executor_name}'",
            tags=["crm", "benchmark", "purple", executor_name],
            examples=['{"task_id":"0","prompt":"Can this lead be qualified?"}'],
        )
    ]
    return AgentCard(
        name="crmarenapro_purple",
        description="Purple agent wrapper for the CRMArenaPro benchmark.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the CRMArenaPro Purple Agent.")
    ...
    LOGGER.info(
        "Starting crmarenapro purple agent with default_executor=%s purple_version=%s executor_version=%s",
        args.executor,
        _resolve_purple_agent_version(),
        _resolve_executor_version(args.executor) or "-",
    )
