...
"""Purple Agent for AutomationBench. Loads the executor dynamically."""
...
def _resolve_purple_agent_version() -> str:
    version = load_component_version(
        Path(__file__).resolve().parent, env_name="BENCHMARK_PURPLE_VERSION"
    )
    if version is None:
        raise RuntimeError("AutomationBench purple agent version is not configured")
    return version


def default_executor_name() -> str:
    return (
        os.getenv("AUTOMATIONBENCH_EXECUTOR")
...
def resolve_executor_version(executor_name: str) -> str | None:
    return load_component_version(
        resolve_executor_dir(executor_name), env_name="BENCHMARK_EXECUTOR_VERSION"
    )
...
def build_agent_card(
    ...
    skill = AgentSkill(
        id=f"automationbench_{executor_name}",
        name=f"automationbench_{executor_name}",
        description=f"AutomationBench purple wrapper for executor '{executor_name}'",
        tags=["automationbench", "benchmark", "purple", "workflow", executor_name],
        examples=['{"task_id":"...","prompt":"...","initial_state":{},"zapier_tools":[]}'],
    )
    return AgentCard(
        name="automationbench_purple",
        description="Purple agent wrapper for the AutomationBench benchmark.",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the AutomationBench Purple Agent.")
    ...
    registry = PurpleExecutorRegistry(
        benchmark_dir=_BENCHMARK_DIR, module_prefix="automationbench"
    )
    ...
    LOGGER.info(
        "Starting routed purple agent with default_executor=%s purple_version=%s "
        "executor_version=%s",
        args.executor,
